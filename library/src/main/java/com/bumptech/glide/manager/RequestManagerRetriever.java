...
public class RequestManagerRetriever implements Handler.Callback {
  ...
  public RequestManager get(@NonNull Context context) {
    if (context == null) {
      ...
      //如果是UI线程也就是主线程，并且context不是Application
  ...
  public RequestManager get(@NonNull FragmentActivity activity) {
    if (Util.isOnBackgroundThread()) {
      ...
      //Activity还没有销毁，不然异常，其实感觉这里不要抛异常比较好，不然存在内存泄露就直接崩溃了，可以取消加载
  ...
  public RequestManager get(@NonNull Fragment fragment) {
    ...
    if (Util.isOnBackgroundThread()) {
      ...
      //Fragment则得到的是getChildFragmentManager
  ...
  private SupportRequestManagerFragment getSupportRequestManagerFragment(
    ...
    //isParentVisible表示Activity是否Finish
    SupportRequestManagerFragment current =
        (SupportRequestManagerFragment) fm.findFragmentByTag(FRAGMENT_TAG);
    if (current == null) {
      current = pendingSupportRequestManagerFragments.get(fm);
      if (current == null) {
        //这里会new一个SupportRequestManagerFragment,一个空的Fragment用于感知Activity的生命周期(观察者模式)
  ...
  private RequestManager supportFragmentGet(
      ...
      boolean isParentVisible) {
    //创建了一个RequestManagerFragment
    SupportRequestManagerFragment current =
        getSupportRequestManagerFragment(fm, parentHint, isParentVisible);
    RequestManager requestManager = current.getRequestManager();
    //创建了一个RequestManager,factory是GeneratedRequestManagerFactory,默认build得到的是GlideRequests
    if (requestManager == null) {
      // TODO(b/27524013): Factor out this Glide.get() call.
      Glide glide = Glide.get(context);
      requestManager =
